...
func (l *LoginHandler) RegisterRoutes(router *gin.RouterGroup) {
	// auth
	router.POST("/login", func(c *gin.Context) { l.login(c.Writer, c.Request) })
	router.GET("/logout", func(c *gin.Context) { l.logout(c.Writer, c.Request) })
